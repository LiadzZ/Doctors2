...
using WebApplication3Forum.Models;
...
namespace WebApplication3Forum.MvcForumIdentity {
	...
	public class MembershipService : IMembershipService {
		...
		public MembershipService(IWebUserProvider userProvider) {
			...
			this.context = new IdentityDbContext("mvcForum.DataProvider.MainDB");
		...
		private IAccount GetAccount(ApplicationUser user) {
            if (user.CreationDate < SqlDateTime.MinValue)
            {
                return new Account
                {
